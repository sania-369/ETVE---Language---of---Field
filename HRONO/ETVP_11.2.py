# =============================================================================
# 🌀 ETVE PURE GEOMETRIC MODEL v11.2
# ТОПОЛОГИЧЕСКАЯ РЕНОРМГРУППА — E8 ИЗНУТРИ
# =============================================================================
# НОВОЕ В v11.2:
# 1. Бета-функции вычисляются из инвариантов Казимира подматриц.
# 2. Массовые члены рождаются из поворота корней (векторов Уолла).
# 3. НЕТ внешних beta_s = 7.0, НЕТ mass_terms = [1, 2, 3].
# 4. ВСЁ выводится из матрицы Картана E8.
# =============================================================================
...
class ETVETopologicalRG:
    """
    🌀 ЕДИНАЯ ТЕОРИЯ ВИХРЕВОГО ПОЛЯ — v11.2
    Топологическая ренормгруппа: E8 изнутри.
    """
    def __init__(self):
    ...
    # =====================================================================
    # 1. ИНВАРИАНТЫ КАЗИМИРА (из подматриц)
    # =====================================================================
    def _casimir_invariants(self, M):
        """
        Вычисляет инварианты Казимира для подгрупп U(1), SU(2), SU(3).
        Инвариант = след(M_sub^2) / след(M_sub)^2
        """
        # Выделяем подматрицы для каждой подгруппы
        # U(1): 1x1 (первый элемент)
        # SU(2): 2x2 (первые 2 элемента)
        # SU(3): 3x3 (первые 3 элемента)
        ...
        return C_U1, C_SU2, C_SU3

    # =====================================================================
    # 2. ДЕФОРМАЦИЯ КОРНЕЙ (массовые члены из поворота)
    # =====================================================================
    def _root_deformation(self, M, C):
        """
        Вычисляет массовые члены из поворота корней (векторов Уолла).
        При C → C_min корни поворачиваются, порождая массы.
        """
        # Вычисляем собственные векторы (корни)
        eigenvalues, eigenvectors = np.linalg.eigh(M)

        # Массовый член для каждой моды — это проекция корня на направление нарушения симметрии
        # Направление нарушения — вектор, соответствующий минимальному собственному значению
        mass_direction = eigenvectors[:, np.argmin(eigenvalues)]
        
        # Массовые члены: проекция каждого корня на направление нарушения
        mass_terms = np.zeros(8)
        for i in range(8):
            # Проекция i-го корня на направление нарушения
            projection = np.dot(eigenvectors[:, i], mass_direction)
            # Масса пропорциональна проекции и степени нарушения (C)
            mass_terms[i] = abs(projection) * (self.C_max - C) / (self.C_max - self.C_min)

        return mass_terms

    # =====================================================================
    # 3. БЕТА-ФУНКЦИИ ИЗ ИНВАРИАНТОВ
    # =====================================================================
    def _compute_betas(self, M):
        """
        Вычисляет бета-функции из инвариантов Казимира.
        beta = 1 / (C - 1/2) (упрощённо, но из геометрии)
        """
        C_U1, C_SU2, C_SU3 = self._casimir_invariants(M)

        # Бета-функции: скорость изменения констант связи
        # При C → C_max (UV) бета-функции малы (симметрия)
        # При C → C_min (IR) бета-функции растут (нарушение)
        beta_em = 1.0 / (C_U1 + 0.5)
        beta_s = 1.0 / (C_SU3 + 0.5)
        beta_w = 1.0 / (C_SU2 + 0.5)

        return beta_em, beta_s, beta_w

    # =====================================================================
    # 4. ПОСТРОЕНИЕ МАТРИЦЫ (с топологическими массами)
    # =====================================================================
    def _build_matrix(self, C):
        """Строит матрицу с массовыми членами из деформации корней."""
        ...
        # Вычисляем массовые члены из деформации корней
        mass_terms = self._root_deformation(M, C)

        # Добавляем массы на диагональ
        for i in range(8):
            M[i, i] += mass_terms[i]
    ...
    # =====================================================================
    # 5. ПОТОК (из топологии)
    # =====================================================================
    def _compute_couplings(self, M, C):
        """Вычисляет константы связи из матрицы и топологических бета-функций."""
        ...
        # Топологические бета-функции
        beta_em, beta_s, beta_w = self._compute_betas(M)

        # Ренормгрупповой поток (из топологии)
    ...
    def run_flow(self, steps=1000):
        print("=" * 80)
        print("   🌀 ETVE v11.2 — ТОПОЛОГИЧЕСКАЯ РЕНОРМГРУППА")
        print("   E8 ИЗНУТРИ: бета-функции и массы из матрицы")
        print("=" * 80)

        C_array = np.linspace(self.C_max, self.C_min, steps)

        for C in C_array:
            M, mass_terms = self._build_matrix(C)
            ...
            self.history["mass_terms"].append(mass_terms.copy())
    ...
    def analyze_flow(self):
        ...
        # Поиск точек пересечения с CODATA
        ...
        print("\n--- РЕЗУЛЬТАТЫ (топологический поток) ---")
        ...
        axes[0, 0].set_title('Константы связи (топологический поток)')
        ...
        axes[1, 0].set_title('Топологические бета-функции')
